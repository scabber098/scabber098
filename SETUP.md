...
```powershell
.\setup.ps1 -Username scabber098 -Name "Your Name" -Image .\me.jpg -Circle -Animate
```

That rewrites `scabber098` / `YOUR NAME` across `README.md` and the workflows, draws the
...
Still to fill in by hand in `README.md`: `https://www.linkedin.com/in/kunwardeep-singh-dev`, `kunwardeepsingh797@gmail.com`, `YOUR_PORTFOLIO`,
`YOUR_CF`, `https://leetcode.com/u/kunwar_7/`, `PROJECT_ONE`…`PROJECT_FOUR`, and the "whoami" bullets.
...
git remote add origin https://github.com/scabber098/scabber098.git
...
```powershell
python scripts\cards.py --user scabber098 --out assets
```
